...
```console
$ helm repo add bigdatacharts https://schub-cloud.github.io/bigdata-platfrom-charts
$ helm upgrade --install my-hadoop bigdatacharts/hadoop
```
...
For more information on installing a Helm chart, please refer to the [Helm documentation] (https://helm.sh/docs/intro/install/).
...
| conf.capacityScheduler | object | `{}` | conf.capacityScheduler this configuration will take effect in the capacity-scheduler.xml file |
...
| singleSetup.enabled | bool | true | If enabled, will setup a master (namenode and resourcemanager) and slave (datanode and nodemanager) statefulsets |
| singleSetup.pdbMinAvailable| int | 3 | The minimum number of PodDisruptionBudget replicas set for slave nodes. Ensure this value is higher or equal to 'conf.hdfsSite.dfs.replication' |
| singleSetup.Replicas | int | 3 | The number of replicas for slave nodes. |
...
Configure Hadoop using the conf parameter in your values.yaml file. The available configuration options are:

- conf.coreSite - Core-site.xml configuration
- conf.hdfsSite - Hdfs-site.xml configuration
- conf.httpfsSite - Httpfs-site.xml configuration
- conf.yarnSite - Yarn-site.xml configuration
- conf.mapredSite - Mapred-site.xml configuration
- conf.capacityScheduler - Capacity-scheduler.xml configuration
